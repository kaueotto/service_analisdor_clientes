...
class dto_modelos:
    ...
    def __str__(self):
        return (
            f"dto_modelos(ModId={self.ModId}, ModCliId='{self.ModCliId}', "
    ...
    def to_dict(self):
        return {
            ...
            'ModDataInclusao': self.ModDataInclusao.strftime(
                '%Y-%m-%d %H:%M:%S'
            )
            if isinstance(self.ModDataInclusao, datetime)
            else self.ModDataInclusao,
        }
