...
from entity import fila, modelos, orm
...
def processa_fila_pedidos():
    ...
    while True:
        try:
            fila_instance = QH.buscar_por_atributo(
                fila.Fila, FilaResultado=None
            )
            if fila_instance:
                fila_dto = dto_fila.dto_fila.from_model(fila_instance)
                modelo_instance = QH.buscar_por_atributo(
                    modelos.Modelos, ModId=fila_dto.FilaModId
                )
                ...
                dados_sacado = json.loads(fila_dto.FilaInfoSacado).get(
                    'dados', None
                )
                ...
                for campo in campos_necessarios:
                    if (
                        campo not in dados_sacado
                        or dados_sacado[campo] is None
                    ):
                        print(
                            f"Erro: O campo '{campo}' está faltando ou é None."
                        )
                        continue
                ...
                if dados_df.isnull().any().any():
                    print(
                        'Erro: Existem valores nulos no DataFrame de entrada.'
                    )
                    continue

                missing_cols = set(modelo_treinado.feature_names_in_) - set(
                    dados_df.columns
                )
                ...
                explainer = LimeTabularExplainer(
                    training_data=dados_df.values,
                    feature_names=modelo_treinado.feature_names_in_,
                    mode='classification'
                    if modelo_treinado.predict(dados_df)
                    else 'regression',
                )

                # explicação para a previsão
                exp = explainer.explain_instance(
                    data_row=dados_df.values[0],
                    predict_fn=modelo_treinado.predict_proba,
                )
                ...
                QH.alterar_registros(
                    ...
                    FilaMotivoResultado=json.dumps(dict(explicacao)),
                )
